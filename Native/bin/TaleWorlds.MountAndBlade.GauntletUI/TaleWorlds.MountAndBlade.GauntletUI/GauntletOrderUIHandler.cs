...
public abstract class GauntletOrderUIHandler : MissionView
{
	...
	protected virtual void TickInput(float dt)
	{
		...
		_dataSource.CanToggleCamera = Input.IsGamepadActive;
		if (Input.IsGamepadActive)
		{
			bool flag2 = false;
			int num = ((IEnumerable<OrderTroopItemVM>)_dataSource.TroopController.TroopList).Count((OrderTroopItemVM t) => ((OrderSubjectVM)t).IsSelectionHighlightActive);
			int num2 = ((IEnumerable<OrderTroopItemVM>)_dataSource.TroopController.TroopList).Count((OrderTroopItemVM t) => ((OrderSubjectVM)t).IsSelected);
			for (int num3 = 0; num3 < ((List<OrderTroopItemVM>)(object)_dataSource.TroopController.TroopList).Count; num3++)
			{
				OrderTroopItemVM val = ((List<OrderTroopItemVM>)(object)_dataSource.TroopController.TroopList)[num3];
				if (!flag2 && ((OrderSubjectVM)val).IsSelectionHighlightActive && ((OrderSubjectVM)val).IsSelectable)
				{
					if (num == 1 && num2 == 1 && ((OrderSubjectVM)val).IsSelected)
					{
						((OrderSubjectVM)val).CanToggleSelection = false;
					}
					else
					{
						((OrderSubjectVM)val).CanToggleSelection = true;
					}
					flag2 = true;
				}
				else
		...
		else
		{
			((List<OrderTroopItemVM>)(object)_dataSource.TroopController.TroopList).ForEach((Action<OrderTroopItemVM>)delegate(OrderTroopItemVM t)
			{
				((OrderSubjectVM)t).IsSelectionHighlightActive = false;
				((OrderSubjectVM)t).CanToggleSelection = true;
			});
		}
		...
		if (_dataSource.IsToggleOrderShown)
		{
			if (_dataSource.ActiveTargetState == 0 && (base.Input.IsKeyReleased((InputKey)224) || base.Input.IsKeyReleased((InputKey)255)))
			{
				if (_dataSource.SelectedOrderSet != null && Input.IsGamepadActive)
				{
					...
					OrderItemVM? obj = ((IEnumerable<OrderItemVM>)_dataSource.SelectedOrderSet.Orders).FirstOrDefault((Func<OrderItemVM, bool>)((OrderItemVM o) => ((OrderItemBaseVM)o).IsSelected));
					if (obj != null)
					{
						((OrderItemBaseVM)obj).ExecuteAction(visualOrderExecutionParameters);
					}
				}
				else
				{
		...
		if (num4 > -1)
		{
			if (_dataSource.SelectedOrderSet != null)
			{
				...
				if (count > 0 && num4 >= 0)
				{
					if (num4 == 8 && ((IEnumerable<OrderItemVM>)_dataSource.SelectedOrderSet.Orders).Any((OrderItemVM x) => x.Order is ReturnVisualOrder))
					{
						_dataSource.SelectedOrderSet.ExecuteDeSelect();
					}
					else if (num4 < count)
					{
						OrderItemVM val2 = ((Collection<OrderItemVM>)(object)_dataSource.SelectedOrderSet.Orders)[num4];
			...
			else
			{
				...
				if (num4 == 8 && ((IEnumerable<OrderSetVM>)_dataSource.OrderSets).Any((OrderSetVM x) => x.HasSingleOrder && ((Collection<OrderItemVM>)(object)x.Orders)[0].Order is ReturnVisualOrder))
				{
					_dataSource.TryCloseToggleOrder(false);
				}
				else
				{
					OrderSetVM orderSetAtIndex = _dataSource.GetOrderSetAtIndex(num4);
		...
		if (!IsDeployment && _dataSource.IsToggleOrderShown && Input.IsGamepadActive)
		{
			...
			{
				_dataSource.OnTroopHighlightSelection((TroopSelectionDirection)0);
			}
			else if (base.Input.IsGameKeyPressed(89))
			{
				_dataSource.OnTroopHighlightSelection((TroopSelectionDirection)1);
			}
			else if (base.Input.IsGameKeyPressed(90))
			{
				_dataSource.OnTroopHighlightSelection((TroopSelectionDirection)2);
			}
			else if (base.Input.IsGameKeyPressed(91))
			{
				_dataSource.OnTroopHighlightSelection((TroopSelectionDirection)3);
			}
		...
		if (num5 != -1)
		{
			_dataSource.OnTroopFormationSelected(num5);
		}
