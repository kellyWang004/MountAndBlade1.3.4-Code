...
public class MissionAgentLabelView : MissionView
{
	...
	private void UpdateProximityBannerTransparencies()
	{
		...
		{
			Agent val = _closeAgentsWithMeshes[i];
			SetBannerHighlightVisibility(val, _agentMeshes[val], IsAgentListeningToOrders(val));
		}
		...
		ProximityMapSearchStruct val2 = AgentProximityMap.BeginSearch(mission, ((Vec3)(ref position)).AsVec2, 8f, false);
		while (((ProximityMapSearchStruct)(ref val2)).LastFoundAgent != null)
		{
			if (_agentMeshes.ContainsKey(((ProximityMapSearchStruct)(ref val2)).LastFoundAgent))
			{
				_closeAgentsWithMeshes.Add(((ProximityMapSearchStruct)(ref val2)).LastFoundAgent);
			}
			AgentProximityMap.FindNext(((MissionBehavior)this).Mission, ref val2);
		}
		for (int j = 0; j < _closeAgentsWithMeshes.Count; j++)
		{
			Agent val3 = _closeAgentsWithMeshes[j];
			SetBannerHighlightVisibility(val3, _agentMeshes[val3], IsAgentListeningToOrders(val3));
		}
	}
	...
	{
		_alwaysShowFriendlyTroopBanners = ManagedOptions.GetConfig((ManagedOptionsType)13) > 1E-05f;
	}
		...
		if (useAllTeamAgents)
		{
			if (PlayerOrderController.Owner != null)
			{
				...
				}
				{
					...
					{
						SetBannerHighlightVisibility(item3, _agentMeshes[item3], highlight);
					}
					return;
				}
			}
