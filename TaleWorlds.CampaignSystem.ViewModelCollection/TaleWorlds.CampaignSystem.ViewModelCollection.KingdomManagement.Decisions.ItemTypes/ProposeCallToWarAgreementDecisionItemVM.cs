...
public class ProposeCallToWarAgreementDecisionItemVM : DecisionItemBaseVM
{
	...
	private Kingdom _calledKingdom => (_decision as ProposeCallToWarAgreementDecision).CalledKingdom;

	public IFaction TargetFaction => (_decision as ProposeCallToWarAgreementDecision).KingdomToCallToWarAgainst;
	...
	public string ProposeCallToWarAgreementDescriptionText
	{
		get
		{
			return _proposeCallToWarAgreementDescriptionText;
		}
		set
		{
			if (value != _proposeCallToWarAgreementDescriptionText)
			{
				_proposeCallToWarAgreementDescriptionText = value;
				OnPropertyChangedWithValue(value, "ProposeCallToWarAgreementDescriptionText");
			}
		}
	}
	...
	{
		_proposeCallToWarAgreementDecision = decision;
		base.DecisionType = 9;
	}
	...
	{
		base.InitValues();
		TextObject textObject = GameTexts.FindText("str_kingdom_decision_propose_call_to_war_agreement");
		NameText = textObject.ToString();
		TextObject textObject2 = GameTexts.FindText("str_kingdom_decision_propose_call_to_war_agreement_desc");
		textObject2.SetTextVariable("CALLED_KINGDOM", _calledKingdom.Name);
		textObject2.SetTextVariable("KINGDOM_TO_CALL_TO_WAR_AGAINST", TargetFaction.Name);
		ProposeCallToWarAgreementDescriptionText = textObject2.ToString();
		SourceFactionBanner = new BannerImageIdentifierVM(_calledKingdom.Banner, nineGrid: true);
		...
		SourceFactionLeader = new HeroVM(_calledKingdom.Leader);
		...
		string faction1Color = Color.FromUint(_calledKingdom.Color).ToString();
		string faction2Color = Color.FromUint(kingdom.Color).ToString();
		KingdomWarComparableStatVM item = new KingdomWarComparableStatVM((int)_calledKingdom.CurrentTotalStrength, (int)kingdom.CurrentTotalStrength, GameTexts.FindText("str_strength"), faction1Color, faction2Color, 10000);
