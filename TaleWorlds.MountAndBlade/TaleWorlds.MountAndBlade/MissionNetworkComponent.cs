...
public sealed class MissionNetworkComponent : MissionNetwork
{
	...
	private void HandleServerEventCreateFreeCorpseMountAgentEvent(GameNetworkMessage baseMessage)
	{
		...
		Agent agent = base.Mission.SpawnMonster(createFreeCorpseMountAgent.HorseItem, createFreeCorpseMountAgent.HorseHarnessItem, createFreeCorpseMountAgent.Position, createFreeCorpseMountAgent.Direction.Normalized(), createFreeCorpseMountAgent.AgentIndex);
	...
	private void HandleServerEventCreateCorpseAgent(GameNetworkMessage baseMessage)
	{
		...
		AgentBuildData agentBuildData = new AgentBuildData(character).MissionPeer(createCorpseAgent.IsPlayerAgent ? missionPeer : null).Monster(createCorpseAgent.Monster).TroopOrigin(new BasicBattleAgentOrigin(character))
			...
			.InitialPosition(createCorpseAgent.Position)
			.InitialDirection(createCorpseAgent.Direction.Normalized())
			.MissionEquipment(createCorpseAgent.MissionEquipment)
			.Team(teamFromTeamIndex)
